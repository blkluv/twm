...
import NFTDropDownMenu, {
  INFTDropDownMenuItem,
} from '../page-ui/nft-dropdown-menu'
...
const NFTCard = ({ nft }: { nft: ProfileNFT }) => {
  ...
  const burnNFT = async (id: string) => {
    setIsBurnOpen(false)
    toast
      .promise(mutateAsync({ tokenId: id }), {
        loading: 'Burning...',
  ...
  const cancelListing = async (id: string) => {
    ...
    toast
      .promise(cancelMutate({ id, type: ListingType.Direct }), {
        loading: 'Canceling...',
 ...
 shadow-md  dark:border dark:border-gray-600"
      key={item.id}
    >
        ...
        {item.isSales ? (
          <>
            ...
            <NFTDropDownMenu menu={menuInSales}>
              <button className="rounded-br-md bg-blue-500 p-1 px-2 text-white hover:bg-blue-400">
                :
              </button>
            </NFTDropDownMenu>
          </>
      ...
      <Dialog
        isOpen={isBurnOpen}
        closeModal={() => setIsBurnOpen(false)}
        title="Are you sure you want to burn this NFT?"
        desc="This action cannot be undone"
        confirm={() => burnNFT(item.id)}
        confirmText="Confirm"
      />
      <Dialog
        isOpen={isCancelOpen}
        closeModal={() => setIsCancelOpen(false)}
        title="Are you sure you want to Cancel this Listing?"
        desc="This action cannot be undone"
        confirm={() => cancelListing(item.listingId as string)}
        confirmText="Confirm"
      />
      <SellDialog
        tokenId={item.id}
        isOpen={isSellOpen}
        closeModal={() => setIsSellOpen(false)}
      />
    </div>
  )
}
