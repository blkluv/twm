...
const UploadForm = () => {
  ...
  const delInput = (index: number) => {
    ...
    setValue(
      'properties',
      properties.filter((item) => item !== properties[index]),
    )
  }
      ...
      <form className="space-y-5" onSubmit={onSubmit}>
        <div>
          ...
          <input
            {...register('name')}
            type="text"
            className="w-[525px] rounded-md border-gray-300 shadow-sm
          ...
          <div className="flex items-center gap-5">
            ...
            {selectedFile && (
              <button
                ...
                className="h-8 rounded border-2 border-red-500 p-1 px-2 text-sm text-red-500 shadow-sm hover:bg-red-50"
              >
                Cancel
              </button>
            )}
        ...
        <div>
            ...
            {properties?.map((item, index) => (
              <div key={index} className="flex items-center space-x-3">
                ...
                <button
                  ...
                  className="rounded-md bg-red-500 text-white shadow-sm hover:bg-red-600"
                >
          ...
          <button
            type="button"
            onClick={addInput}
            className="mt-3 flex items-center rounded bg-green-500 p-1 text-sm text-white shadow-sm hover:bg-green-400"
          >
            Add row
          </button>
        </div>
        <div className="space-x-5">
          <button
            disabled={isLoading}
            type="submit"
            className="rounded bg-blue-500 p-2 px-6 text-white shadow-sm hover:bg-blue-400"
          >
            Submit
          </button>
          <button
            type="reset"
            onClick={onReset}
            className="rounded bg-gray-200 p-2 px-4 shadow-sm hover:bg-gray-300"
          >
            reset
          </button>
        </div>
