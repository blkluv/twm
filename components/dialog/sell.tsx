...
const SellDialog = ({ isOpen, closeModal, tokenId }: ISellDialogProps) => {
  ...
  return (
    <BaseDialog
      ...
      title="Would you wang to sell it"
      desc="Please input the price"
    >
      <form className="mt-4 flex flex-col">
        <label className="flex items-center overflow-hidden rounded-xl bg-gray-200 shadow-sm">
          <input
            ref={ref}
            type="number"
            className="flex-1 rounded-l-xl border-gray-300 shadow-sm
                    focus:border-l-blue-500 focus:ring focus:ring-blue-200/50"
            placeholder="0.01 - 99999"
          />
          <span className="pl-4 pr-8">ETH</span>
        </label>
